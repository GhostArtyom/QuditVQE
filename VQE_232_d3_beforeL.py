...
def running(model: int, num: int, repeat: Union[int, range, List[int]], layers: int = 2):
    ...
    path = f'./data_232/beforeL'  # path of folder
    s = loadmat(f'{path}/232_beforeL_d3_D1_model{model}_RDM2_iter{num}_target_state_vector.mat')
    state = s['target_state_vec'].flatten()
    D, N = 1, 2  # bond dimension, number of qudits

    log = f'./data_232/Logs/beforeL_L{layers}.log'
...
for model in [1216, 1705]:
    ...
    for repeat in range(1, repetitions + 1):
        ...
        info(f'model{model} D1 repeat{repeat} finish\n{eval_dict}\n{time_dict}\n{fidelity_dict}')
        print(f'model{model} D1 repeat{repeat} finish')
